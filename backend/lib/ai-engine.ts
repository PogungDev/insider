...
Analyze the following wallet data and provide insights, including rugpull risk from dev accumulation and post-unlock dump probability:
...
Dev Accumulation: ${walletData.devAccumulation || 'Unknown'}% of supply (high risk if >30%)

Based on this data, provide a JSON response with:
1. summary: A concise analysis of wallet behavior pattern (max 100 words)
2. riskScore: Risk level from 0.0 (very low) to 1.0 (very high), factoring in rugpull and unlock risks
3. recommendation: One of "Hold", "Sell", "Hedge", or "Buy More"
4. reasoning: Explanation for the recommendation, including predictive investment strategies and risk mitigations (max 150 words)
5. confidence: Confidence level from 0.0 to 1.0
6. rugpullRisk: Probability of rugpull (0-1)
7. dumpProbability: Post-unlock dump probability (0-1)
