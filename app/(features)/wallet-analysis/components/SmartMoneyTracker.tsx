...
export function SmartMoneyTracker() {
  ...
  useEffect(() => {
    // Filter data based on target wallet if available
    if (targetWallet) {
      // Show data related to the target wallet
      const filteredMoves = mockRecentMoves.filter(move => 
        move.walletAddress === targetWallet || 
        mockSmartWallets.some(wallet => wallet.address === targetWallet)
      )
      setRecentMoves(filteredMoves.length > 0 ? filteredMoves : mockRecentMoves)
      
      // Add target wallet to smart wallets if not already there
      const targetWalletData = mockSmartWallets.find(w => w.address === targetWallet)
      if (!targetWalletData && analysisData) {
        const newWalletData: SmartMoneyWallet = {
          id: "target",
          address: targetWallet,
          label: "Target Wallet",
          smartScore: analysisData.behaviorScore || 0,
          totalPnL: analysisData.totalVolume || 0,
          winRate: 75,
          avgHoldTime: 48,
          totalVolume: analysisData.totalVolume || 0,
          followersCount: 0,
          recentActivity: "Currently being analyzed",
          riskLevel: analysisData.riskLevel || 'medium',
          specialization: ["Analysis Target"],
          lastActive: Date.now()
        }
        setSmartWallets([newWalletData, ...mockSmartWallets])
      } else {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Follow and copy trade from the best smart money wallets</p>
        </div>
