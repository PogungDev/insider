...
const LiquidityScanner: React.FC = () => {
  ...
  const loadLiquidityData = () => {
    ...
    // Add wallet-specific liquidity pools if targetWallet is available
    if (targetWallet && analysisData) {
      const walletSpecificPool: LiquidityPool = {
        id: `pool-wallet-${targetWallet.slice(-8)}`,
        protocol: 'Target Wallet Pools',
        tokenA: 'SEI',
        tokenB: 'WALLET',
        tvl: analysisData.totalValue || 100000,
        volume24h: (analysisData.totalValue || 100000) * 0.1,
        liquidity: (analysisData.totalValue || 100000) * 0.8,
        utilization: analysisData.riskScore || 50,
        riskScore: 100 - (analysisData.riskScore || 50),
        riskLevel: analysisData.riskScore > 80 ? 'low' : analysisData.riskScore > 60 ? 'medium' : 'high',
        lastUpdated: new Date(),
        metrics: {
          impermanentLoss: analysisData.riskScore > 70 ? 2.5 : 8.5,
          slippage: analysisData.riskScore > 70 ? 0.2 : 1.5,
          depth: analysisData.riskScore || 50,
          concentration: 100 - (analysisData.riskScore || 50),
          volatility: analysisData.riskScore > 70 ? 15 : 35,
          apy: analysisData.riskScore > 70 ? 20 : 45
        },
        alerts: analysisData.riskScore < 60 ? [
          {
            id: 'wallet-alert-1',
            type: 'volatility',
            severity: 'high',
            message: `High volatility detected in wallet ${targetWallet.slice(0, 6)}...${targetWallet.slice(-4)} liquidity positions`,
            timestamp: new Date(),
            resolved: false
          }
        ] : []
      };
      mockPools.unshift(walletSpecificPool);
    }

    let mockEvents: LiquidityEvent[] = [
      {
        id: 'event-1',
        poolId: 'pool-1',
        type: 'remove',
    ...
    // Add wallet-specific events if targetWallet is available
    if (targetWallet && analysisData) {
      const walletEvent: LiquidityEvent = {
        id: `event-wallet-${targetWallet.slice(-8)}`,
        poolId: `pool-wallet-${targetWallet.slice(-8)}`,
        type: 'swap',
        amount: (analysisData.totalValue || 50000) * 0.1,
        impact: analysisData.riskScore > 70 ? 5 : 15,
        timestamp: new Date(Date.now() - 10 * 60 * 1000),
        txHash: targetWallet,
        suspicious: analysisData.riskScore < 60
      };
      mockEvents.unshift(walletEvent);
    }
