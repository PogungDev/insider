...
export function AlphaRanking() {
  ...
  useEffect(() => {
    // Add target wallet related signals if available
    if (targetWallet && analysisData) {
      // Create target wallet specific signal
      const targetSignal: AlphaSignal = {
        id: 'target-wallet-signal',
        rank: 0,
        title: `Target Wallet Analysis - ${targetWallet.slice(0, 8)}...`,
        type: 'investment',
        confidence: analysisData.behaviorScore || 75,
        expectedReturn: 25.8,
        timeframe: '1-2 weeks',
        riskLevel: analysisData.riskLevel || 'medium',
        source: 'Wallet Behavior Analysis',
        description: 'Analysis of target wallet showing consistent alpha patterns and strategic positioning.',
        tags: ['target-analysis', 'behavioral', 'alpha-wallet'],
        createdAt: Date.now(),
        expiresAt: Date.now() + 86400000 * 14,
        followers: 0,
        successRate: 82.5,
      ...
      // Adjust ranks of existing signals
      const adjustedSignals = mockAlphaSignals.map(signal => ({
        ...signal,
        rank: signal.rank + 1
      }))
      
      setAlphaSignals([targetSignal, ...adjustedSignals])
    } else {
