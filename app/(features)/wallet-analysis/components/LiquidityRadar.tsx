...
export function LiquidityRadar() {
  ...
  const loadLiquidityData = () => {
    ...
    // Add wallet-specific liquidity pool if targetWallet is available
    if (targetWallet && analysisData) {
      const walletPool: LiquidityPool = {
        id: `wallet-pool-${targetWallet.slice(-8)}`,
        name: "SEI/USDT",
        token0: "SEI",
        token1: "USDT",
        tvl: (analysisData.totalValue || 500000) * 2,
        volume24h: (analysisData.totalValue || 500000) * 0.8,
        fees24h: (analysisData.totalValue || 500000) * 0.003,
        apy: analysisData.riskScore > 70 ? 52.3 : 18.7,
        liquidityChange24h: analysisData.riskScore > 70 ? 12.8 : -8.2,
        riskScore: 100 - (analysisData.riskScore || 75),
        concentration: analysisData.riskScore || 75,
        stability: analysisData.riskScore > 70 ? 92 : 65
      }
      mockPools.unshift(walletPool)
    }

    let mockFlowEvents: FlowEvent[] = [
      {
        id: "1",
        type: "remove",
        pool: "SEI/USDC",
    ...
    // Add wallet-specific flow events if targetWallet is available
    if (targetWallet && analysisData) {
      const walletFlowEvent: FlowEvent = {
        id: `wallet-flow-${targetWallet.slice(-8)}`,
        type: analysisData.riskScore > 70 ? 'add' : 'remove',
        pool: "SEI/USDT",
        amount: (analysisData.totalValue || 100000) * 0.4,
        timestamp: Date.now() - 120000,
        wallet: `${targetWallet.slice(0, 6)}...${targetWallet.slice(-3)}`,
        impact: analysisData.riskScore > 70 ? 2.8 : 4.1,
        isWhale: (analysisData.totalValue || 100000) > 500000
      }
      mockFlowEvents.unshift(walletFlowEvent)
    }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Real-time monitoring of liquidity flow and market depth</p>
        </div>
